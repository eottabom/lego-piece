...
* 불변성이 보장되는 변수만 최적화 가능
  ...
  - 불변성 자체는 객체가 아닌 기본 데이터 타입에 대해서는 큰 이점을 제공하지 않을 수 있다.

* 자바 메모리 모델과 관련된 제약
  - Java 메모리 모델은 멀티스레드 환경에서의 변수 접근을 제어하기 위해 여러 규칙을 가지고 있다.
  - final 키워드는 변수가 초기화된 후에 수정할 수 없음을 보장하지만, 이는 멀티스레드 환경에서의 변수 접근 최적화와는 별개의 문제일 수 있다.
